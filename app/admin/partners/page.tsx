...
export default function PartnersPage() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch partner leads",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const updateLeadStatus = async (leadId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/admin/partners/${leadId}`, {
        method: 'PATCH',
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2">Corporate Partnership Inquiries</h1>
          <p className="text-muted-foreground">
            Manage and track partnership applications
          </p>
        </div>
      </div>
          ...
          {selectedLead && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium">Tier Interest</label>
                  <p className="text-lg">{selectedLead.tier}</p>
                </div>
              </div>
